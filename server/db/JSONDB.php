...
class JSONDB {
	...
	public static function condition( string $value, string $condition = ">" ) : object {
		$c = new \stdClass();
	...
	private function intersect_value_check($a, $b) {		
		if( $b instanceof \stdClass ) {
			...
			if( $b->is_conditional ) {
				return !$this->checkCondition($b->value,$b->condition,$a);
			}
		...
		if( $a instanceof \stdClass ) {
			...
			if( $b->is_conditional ) {
				return !$this->checkCondition($a->value,$a->condition,$b);
			}
